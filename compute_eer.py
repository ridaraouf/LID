...
def load_file(score, trial):
    ...
    if score_all.size % 2 != 0:
        logging.warning(f"Cannot reshape score_all of size {score_all.size} into shape (-1, 2). Padding with zeros.")
        score_all = np.pad(score_all, (0, 2 - score_all.size % 2), mode='constant')
